...
//         >
//           <div
//             className={`w-full max-w-xl rounded-2xl border ${dark ? "bg-[#17161a] border-white/[0.1]" : "bg-white border-black/[0.1]"
...
//                 </button>
//               )}
//               <kbd className={`hidden sm:block text-[10px] font-mono px-1.5 py-0.5 rounded border ${dark ? "border-white/20 text-white/30" : "border-black/20 text-black/30"
//                 }`}>
//                 Esc
//               </kbd>
//             </div>
...
//                     key={mode}
//                     onClick={() => setViewMode(mode)}
//                     className={`p-1.5 rounded-lg transition-all ${viewMode === mode ? `${accentBg} text-[#0f0f12]` : `${sub} ${surfaceHov}`
//                       }`}
//                   >
//                     <Icon size={16} />
//                   </button>
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${bg} ${borderCls} transition-colors duration-150`}>
        ...
        <input
          ...
          className={`flex-1 bg-transparent outline-none text-sm ${txt} placeholder-[#5a5652]`}
        />
        {rightElement}
      </div>
      {error && (
    ...
    <ModalBackdrop dark={dark} onClose={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          ...
          <InputField label="Full Name" type="text" value={name} onChange={(v) => { setName(v); setErrors(e => ({ ...e, name: undefined })); }} placeholder="Alex Morgan" icon={User} error={errors.name} dark={dark} />
          <InputField label="Email Address" type="email" value={email} onChange={(v) => { setEmail(v); setErrors(e => ({ ...e, email: undefined })); }} placeholder="alex@example.com" icon={Mail} error={errors.email} dark={dark} />
          <div className="flex flex-col gap-1.5">
            <InputField label="Password" type={showPass ? "text" : "password"} value={password} onChange={(v) => { setPassword(v); setErrors(e => ({ ...e, password: undefined })); }} placeholder="Min. 8 characters" icon={Lock} error={errors.password} dark={dark}
          ...
          <InputField label="Confirm Password" type={showConf ? "text" : "password"} value={confirm} onChange={(v) => { setConfirm(v); setErrors(e => ({ ...e, confirm: undefined })); }} placeholder="Re-enter your password" icon={ShieldCheck} error={errors.confirm} dark={dark}
          ...
          <p className={`text-[11px] leading-relaxed ${sub} text-center px-2`}>
            By creating an account you agree to our <span className="text-[#e8a44a] cursor-pointer hover:underline">Terms</span> and <span className="text-[#e8a44a] cursor-pointer hover:underline">Privacy Policy</span>.
          </p>
        </div>
    ...
    <ModalBackdrop dark={dark} onClose={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <InputField label="Email Address" type="email" value={email} onChange={(v) => { setEmail(v); setErrors(e => ({ ...e, email: undefined })); }} placeholder="alex@example.com" icon={Mail} error={errors.email} dark={dark} />
          <InputField label="Password" type={showPass ? "text" : "password"} value={password} onChange={(v) => { setPassword(v); setErrors(e => ({ ...e, password: undefined })); }} placeholder="Your password" icon={Lock} error={errors.password} dark={dark}
            rightElement={<button type="button" onClick={() => setShowPass(!showPass)} className={`${sub} hover:text-[#e8a44a] transition-colors`}>{showPass ? <EyeOff size={15}/> : <Eye size={15}/>}</button>}
          />
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 gap-3">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div className={`hidden sm:flex items-center p-1 rounded-xl ${surface} gap-0.5`}>
                {viewOptions.map(({ mode, Icon }) => (
                  <button key={mode} onClick={() => setViewMode(mode)} title={`${mode} view`}
                    className={`p-1.5 rounded-lg transition-all duration-150 ${viewMode === mode ? `${accentBg} text-[#0f0f12] shadow-sm` : `${sub} ${surfaceHov}`}`}>
                    <Icon size={16}/>
                  </button>
                ))}
              </div>
              ...
              {/* notifications */}
              <button title="Notifications" className={`relative p-2 rounded-xl ${surface} ${surfaceHov} transition-colors hidden sm:flex`} aria-label="Notifications">
                <Bell size={18}/>
                {notifCount > 0 && (
                  <span className={`absolute top-1 right-1 min-w-[16px] h-4 px-1 rounded-full text-[9px] font-bold flex items-center justify-center ${accentBg} text-[#0f0f12]`}>{notifCount}</span>
                )}
              </button>

              {/* pinned */}
              <button title="Pinned notes" className={`relative p-2 rounded-xl ${surface} ${surfaceHov} transition-colors hidden lg:flex`} aria-label="Pinned notes">
                <Pin size={17}/>
              </button>

              {/* new note */}
              <button className={`hidden sm:flex items-center gap-2 px-3.5 py-2 rounded-xl ${accentBg} text-[#0f0f12] text-sm font-semibold transition-all hover:opacity-90 active:scale-95`}
                style={{ boxShadow: "0 4px 16px rgba(232,164,74,0.25)" }}>
                <PenLine size={15}/>
                <span className="hidden lg:block">New Note</span>
              </button>
